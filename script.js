...
function render() {
  ...
  document.querySelector('#recipeCards').innerHTML = state.recipes.map(r => { const cost=recipeCost(r), food=cost/r.price*100; return `<article class="recipe"><h3>${r.name}</h3><span class="cost">${money(cost)}</span> <small>per serving</small><p>Menu price ${money(r.price)} · Food cost ${food.toFixed(1)}%</p><div class="meter"><i style="width:${Math.min(food,100)}%"></i></div><p>${r.ingredients.map(x=>`${x.qty} ${findItem(x.name)?.unit||''} ${x.name}`).join(' · ')}</p></article>`; }).join('') || '<p class="empty">No recipes yet.</p>';
